...
meRouter.get("/", async (req, res) => {
  ...
  try {
    ...
    const err: ApiErrorResponse = { code: "INTERNAL", message: "Failed to fetch profile" };
...
meRouter.patch("/", async (req, res) => {
  ...
  try {
    ...
    const err: ApiErrorResponse = { code: "INTERNAL", message: "Failed to update profile" };
    res.status(500).json(err);
  }
});
...
meRouter.post("/password", async (req, res) => {
  ...
  try {
    ...
    const err: ApiErrorResponse = { code: "INTERNAL", message: "Failed to change password" };
    res.status(500).json(err);
  }
});
